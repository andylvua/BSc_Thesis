...
To evaluate the end-to-end performance of the proposed IPS pipeline, which integrates A-REMNet-based error mitigation with EKF-based localization, we conducted a series of trajectory reconstruction experiments under varying conditions. Three scenarios were tested: (i) no obstacles (clear LoS), (ii) obstructed paths in a known (seen) environment used during dataset collection (see \autoref{data_collection}), and (iii) obstructed paths in a previously unseen environment. Each scenario included two movement patterns: a rectangular loop and an hourglass-shaped trajectory.
...
\begin{figure}[tbh]
    ...
    % \includegraphics[height=6.5cm]{Graphics/evaluation_env.JPG}
...
% \begin{equation}
%     \mathcal{W} = \{ (i_k, j_k) \mid i_k \in [1, N],\ j_k \in [1, M] \}_{k=1}^K,
% \end{equation}
...
% \begin{table}[tbh]
% \centering
% \caption{Median MAE and relative improvement for each setup. All values in centimeters. $\pm$ denotes standard deviation.}
% \label{tab:mae-improvement}
% \begin{tabular}{llccc}
% \toprule
% \textbf{Setup} & \textbf{Trajectory} & \textbf{MAE} (raw) & \textbf{MAE} (mitigated) & \textbf{Improvement} \\
% \midrule
...
% \begin{figure}[tbh]
%     \centering
%     \includegraphics[width=0.8\textwidth]{Graphics/los_trajectory_example.pdf}
%     \caption{Qualitative illustration of trajectory reconstruction with and without mitigation under LoS conditions. Annotated arrows highlight key regions where systematic deviation was effectively corrected.}
%     \label{fig:trajectory_los}
% \end{figure}

% \begin{figure}[tbh]
%     \centering
%     \includegraphics[width=0.8\textwidth]{Graphics/dual_trajectory_plot.pdf}
%     \caption{Qualitative illustration of trajectory reconstruction in the unseen environment under NLoS conditions.}
%     \label{fig:trajectory_unseen_nlos}
% \end{figure}

%A.Y.: New variant: seems to fit better, but may be a little too small...
\begin{figure}[hp]
    \centering
    \subfloat[LoS conditions. Arrows highlight key regions where systematic deviation was effectively corrected.\label{fig:trajectory_los}]{
        \includegraphics[width=0.72\textwidth]{Graphics/los_trajectory_example.pdf}
    }\\
    \subfloat[NLoS conditions. Unseen environment.\label{fig:trajectory_unseen_nlos}]{
        \includegraphics[width=0.72\textwidth]{Graphics/dual_trajectory_plot.pdf}
    }
    \caption[Qualitative illustration of trajectory reconstruction.]{Qualitative illustration of trajectory reconstruction with and without error mitigation.}
    \label{fig:trajectory_combined}
\end{figure}

To illustrate typical performance visually, we present qualitative comparisons of raw versus mitigated estimates against ground truth, for the LoS and unseen NLoS configurations in \autoref{fig:trajectory_los}, and \autoref{fig:trajectory_unseen_nlos}, respectively. 
